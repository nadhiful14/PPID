	...
	protected function change_list_value($field_info, $value = null)
	{
		...
		switch ($real_type) {
			...
			case 'integer':
				break;
			case 'rupiah':
				$value = "<div class='rupiah'  style='text-align:right'>$value</div>";
				break;
			case 'nomor_pisah':
				$value = "<div class='nomor_pisah' style='text-align:right'>$value</div>";
				break;
			case 'true_false':
			...
			case 'upload_file':
				if (empty($value)) {
					...
					$is_image = !empty($value) && (substr($value, -4) == '.jpg'
						|| substr($value, -4) == '.png'
						|| substr($value, -5) == '.jpeg'
						|| substr($value, -4) == '.gif'
						|| substr($value, -5) == '.tiff')
					...
					if (!empty($value)) {
						if (substr($value, -4) == '.pdf') {
							$iconFile = "<i style='font-size:32px !important' class='fa fa-file-pdf-o'></i>";
						} else if (substr($value, -4) == '.xls' || substr($value, -5) == '.xlsx') {
							$iconFile = "<i style='font-size:32px !important' class='fa fa-file-excel-o'></i>";
						} else if (substr($value, -4) == '.doc' || substr($value, -5) == '.docx') {
							$iconFile = "<i style='font-size:32px !important' class='fa fa-file-word-o'></i>";
						} else {
							$iconFile = "<i style='font-size:32px !important' class='fa fa-folder-o'></i>";
						}
					}
					$file_url = base_url() . $field_info->extras->upload_path . "/$value";
					$file_url_anchor = '';
					if ($is_image) {
						$file_url_anchor .= '<a href="javascript:void(0)" class="image-thumbnail popup"><img src="' . $file_url . '" width="70px">';
					} else {
						$file_url_anchor .= '<a href="' . $file_url . '" target="_blank">' . $iconFile;
					}
					$file_url_anchor .= '</a>';
	...
	protected function db_update($state_info)
	{
		...
		if ($validation_result->success) {
			...
			if ($this->callback_update === null) {
				...
				$update_data = array();
				$types = $this->get_field_types();
				//tambahanku include field primary key auto increment=> karena awalnya auto increment ndak dimasukkan kedalam data yang akan diupdate
				foreach ($types as $keyTypes => $valType) {
					if ($valType->primary_key == 1 && $valType->db_extra == "auto_increment") {
						$update_data[$keyTypes] = $primary_key;
						break; //ketika auto increment sudah ketemu keluar dari perulangan
					}
				}
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected $default_true_false_text 	= array("Tidak", 'Benar');
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->order_by				= $this->order_by;
		//tambahanku 
		$data->multi_button			= $this->multi_button;
		$data->button_side_add		= $this->button_side_add;
		...
		foreach ($data->list as $num_row => $row) {
			//tambahanku sampe sini
			$paramsPrimary = "";
			//jika pakek ini ketika pakek view dengan custom primary key ndak bisa
			// foreach ($data->primary_key as $key_pk => $pk) {
			// 	$paramsPrimary .= $row->{$pk} . "/";
			// }
			if (is_array($data->primary_key)) {
				foreach ($data->primary_key as $key_pk => $pk) {
					$paramsPrimary .= $row->{$pk} . "/";
				}
			} else {
				$paramsPrimary = $row->{$data->primary_key};
			}
			$data->list[$num_row]->primary_key_value = "/" . $paramsPrimary;
			$data->list[$num_row]->edit_url = $data->edit_url . '/' . $paramsPrimary;
			$data->list[$num_row]->delete_url = $data->delete_url . '/' . $paramsPrimary;
			$data->list[$num_row]->read_url = $data->read_url . '/' . $paramsPrimary;
			$data->list[$num_row]->clone_url = $data->clone_url . '/' . $paramsPrimary;
	...
	protected function change_list_add_actions($list)
	{
		...
		foreach ($list as $num_row => $row) {
			$actions_urls = array();
			//tambahanku
			foreach ($primary_key as $key => $pk) {
				$arrayPrimaryKey .= $row->{$pk} . "/";
			}
			foreach ($this->actions as $unique_id => $action) {
				if (!empty($action->url_callback)) {
					$actions_urls[$unique_id] = call_user_func($action->url_callback, $arrayPrimaryKey, $row);
					// $actions_urls[$unique_id] = call_user_func($action->url_callback, $row->$primary_key, $row);
				} else {
					$actions_urls[$unique_id] =
						$action->url_has_http ?
						$action->link_url . $arrayPrimaryKey : site_url($action->link_url . '/' . $arrayPrimaryKey);
					// $action->link_url . $arrayPrimaryKey : site_url($action->link_url . '/' . $row->$primary_key);
				}
			}
	...
	protected function get_integer_input($field_info, $value)
	{
		...
		//tambahanku if primary when on edit jangan bisa diedit
		$readonly = ($field_info->primary_key == 1) ? "readonly" : "";
		$setHiddenAfter = ($field_info->primary_key == 1) ? "<input name='{$field_info->name}' type='hidden' value='$value'>" : "";
		$getStateCode = $this->getStateCode();
		if ($getStateCode == 2) { //jika == 2 maka hapus readonly sama hidden afternya
			$readonly = "";
			$setHiddenAfter = "";
		}
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$value' class='numeric form-control' $extra_attributes $readonly/> $setHiddenAfter";
		return $input;
	}
	...
	protected function get_string_input($field_info, $value)
	{
		...
		$input = "<input id='field-{$field_info->name}' class='form-control' name='{$field_info->name}' type='text' value=\"$value\" $extra_attributes $readonly/> $setHiddenAfter";
		return $input;
	}
	...
	//tambahan inputan rupiah
	protected function get_rupiah_input($field_info, $value)
	{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$value' class='rupiah form-control' />";
		return $input;
	}
	protected function get_nomor_pisah_input($field_info, $value)
	{
		$this->set_js_lib($this->default_javascript_path . '/jquery_plugins/jquery.inputmask.bundle.js');
		$this->set_js_config($this->default_javascript_path . '/jquery_plugins/config/jquery_input_mask.js');
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$value' class='nomor_pisah form-control' />";
		return $input;
	}
	...
	protected function get_date_input($field_info, $value)
	{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$date' maxlength='10' class='datepicker-input form-control' $readonly/>
		<a class='datepicker-input-clear' tabindex='-1'>" . $this->l('form_button_clear') . "</a> (" . $this->ui_date_format . ") $setHiddenAfter";
		return $input;
	}
	...
	protected function get_dropdown_input($field_info, $value)
	{
		...
		$input = "<select $readonly id='field-{$field_info->name}' name='{$field_info->name}' class='chosen-select' data-placeholder='" . $select_title . "'>";
	...
	protected function get_set_input($field_info, $value)
	{
		...
		$input = "<select $readonly id='field-{$field_info->name}' name='{$field_info->name}[]' multiple='multiple' size='8' class='chosen-multiple-select' data-placeholder='$select_title' style='width:510px;' >";
	...
	protected function get_relation_input($field_info, $value)
	{
		...
		$select_title = str_replace('{field_display_as}', $field_info->display_as, $this->l('set_relation_title'));
		$input = "<select $readonly id='field-{$field_info->name}'  name='{$field_info->name}' class='$ajax_or_not_class' data-placeholder='$select_title' style='width:300px'>";
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$input .= $is_image ? " $image_class'><img src='" . $file_url . "' width='70px'>" : "' target='_blank'>$value";
		$input .= "</a> ";
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	{
		//tambahanku
		// $getPrimaryKey = $this->get_primary_key();
		...
		switch ($state_code) {
			...
			case self::STATE_READ:
				if ($first_parameter !== null) {
					...
					$state_info = (object) array('primary_key' => $segment_object->array_primary);
				} else {
	...
	protected function get_state_info_from_url()
	{
		...
		foreach ($segements as $num => $value) {
			//tambahanku tampung primary key
			if ($startPrimary >= 2) { //3 karena pertama diset 1 trus ketika operasi ditemukan jadi 2 nah primarykey adalah setelah operasi itu
				$arrayPrimary[] = $value;
			}
		...
		return (object) array('array_primary' => $arrayPrimary, 'segment_position' => $segment_position, 'operation' => $operation, 'first_parameter' => $first_parameter, 'second_parameter' => $second_parameter);
	}
	...
	protected function getValidationUpdateUrl($primary_key = null)
	{
		...
		return $this->state_url('update_validation/' . $primary);
	}
	...
	protected function getReadUrl($primary_key = null)
	{
		if ($primary_key === null)
			...
			$primary = implode("/", $primary_key);
		return $this->state_url('read/' . $primary);
	}

	protected function getUpdateUrl($state_info)
	{
		//tambahanku
		$primary = implode("/", $state_info->primary_key);
		return $this->state_url('update/' . $primary);
	...
	protected function getDeleteUrl($state_info = null)
	{
		if (empty($state_info)) {
			...
			//tambahanku
			$primary = implode("/", $state_info->primary_key);
			return $this->state_url('delete/' . $primary);
		}
	}
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	//tambahanku
	protected $multi_button			= array();
	protected $button_side_add		= array();
	...
	//tambahanku
	public function composite_key($array = null)
	{
		foreach ($array as $key => $val) {
			$this->composite_key[] = $val;
		}
		return $this;
	}
	 ...
	 * @param $url_callback
	 */
	public function add_action($label, $image_url = '', $link_url = '', $css_class = '', $url_callback = null, $type = null, $message = null)
	{
		...
		$this->actions[$unique_id]  = (object) array(
			...
			'message'		=> $message,
			'type'			=> $type, //emodal_type,confirm_type, ajax_type,default_type
			'url_has_http'	=> substr($link_url, 0, 7) == 'http://' || substr($link_url, 0, 8) == 'https://' ? true : false
		);

		return $this;
	}
	public function add_multi_button($label,  $link_url = '', $css_class = '', $with_confirm = null, $title_confirm = null, $message_confirm = null, $type = null)
	{
		$unique_id = substr($label, 0, 1) . substr(md5($label . $link_url), -8); //The unique id is used for class name so it must begin with a string
		$this->multi_button[$unique_id]  = (object) array(
			'label' 		=> $label,
	...
	public function add_button_side_add($label = null, $link = null, $css_class = null, $confirm = null, $type = null)
	{
		$unique_id = substr($label, 0, 1) . substr(md5($label . $link), -8); //The unique id is used for class name so it must begin with a string
		$this->button_side_add[$unique_id]  = (object) array(
			'label' 		=> $label,
...
class UploadHandler
{
	...
	function getFullUrl()
	{
		return (isset($_SERVER['HTTPS']) ? 'https://' : 'http://') . (isset($_SERVER['REMOTE_USER']) ? $_SERVER['REMOTE_USER'] . '@' : '') . (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : ($_SERVER['SERVER_NAME'] . (isset($_SERVER['HTTPS']) && $_SERVER['SERVER_PORT'] === 443 ||
