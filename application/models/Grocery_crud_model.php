...
class Grocery_crud_model  extends CI_Model  {
    ...
    function get_edit_values($primary_key_value)
    {
		...
		//tambahanku
		$whereTampil = array_combine($primary_key_field,$primary_key_value);
    	$this->db->where($whereTampil);
    ...
    function db_update($post_array, $primary_key_value)
    {
		$primary_key_field = $this->get_primary_key();
		//tambahanku
		// if(count($primary_key_field)>1){
			foreach($primary_key_field as $key=>$val){
				$whereUpdate[$val]=$post_array[$val];
			}
			return $this->db->update($this->table_name,$post_array, $whereUpdate);
		// }else{
			//jika primarikeynya cuman satu
			// return $this->db->update($this->table_name,$post_array, array( $primary_key_field[0] => $primary_key_value));
		// }
    }
    ...
    function db_delete($primary_key_value)
    {
		...
		if(is_array($primary_key_value)){
			//jika array diklik dari tombol satu satu
			$whereDelete =array_combine($primary_key_field,$primary_key_value);
		}else{
			//diakses dari checkbox multiple delete
			$primaryUrl = explode("/",$primary_key_value);
			unset($primaryUrl['0']);
			unset($primaryUrl[count($primaryUrl)]);
			$whereDelete = array_combine($primary_key_field,$primaryUrl);
		}
    	$this->db->delete($this->table_name,$whereDelete);
    	...
    	if($table_name == null)
    	{
	    	...
	    	if(empty($this->primary_key))
	    	{
		    	...
		    	foreach($fields as $field)
		    	{
		    		...
		    		{
		    			$composite_key[]=$field->name;
		    		}
				}
				if(count($composite_key)>0){
