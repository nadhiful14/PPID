<?php if(!empty($notifBaru)){ ?>
<!-- <li class="nav-item dropdown pesan-notif"> -->
    <a href="#" data-toggle="dropdown" role="button" aria-expanded="false" class="nav-link dropdown-toggle"><i class="educate-icon educate-message edu-chat-pro" aria-hidden="true"></i><span class="indicator-ms"></span></a>
    <div role="menu" class="author-message-top dropdown-menu animated zoomIn">
        <div class="message-single-top">
            <h1 style="margin-left: 18px;text-align:left !important;font-size: 14px;font-weight: bold;display: block;">Pengajuan SKPI BARU</h1>
        </div>
        <ul class="message-menu" style="margin-top:0px !important;overflow:auto;">
        <?php  
            foreach($notifBaru as $key=>$val){
                $jenis = $val->jenis_id;
                //gen gambar
                if($jenis==1){
                    $gambar = 'prestasi.png';
            ...
            <li style="width:100% !important">
                <a href="javascript:void(0)" onclick="emodal('<?php echo site_url('admin/notif_skpi/'.$val->id) ?>','SKPI Baru')">
