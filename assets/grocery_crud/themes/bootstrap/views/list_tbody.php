...
<?php foreach ($list as $num_row => $row) { ?>
    <tr>
        <td <?php if ($unset_delete) { ?> style="border-right: none;" <?php } ?> <?php if ($buttons_counter === 0) { ?>class="hidden" <?php } ?>>
            <?php if (!$unset_delete) { ?>
                    ...
                    <?php if ($show_more_button) { ?>
                        <div class="btn-group dropdown">
                            ...
                            <ul class="dropdown-menu">
                                <?php
                                            if (!empty($row->action_urls)) {
                                                foreach ($row->action_urls as $action_unique_id => $action_url) {
                                                    ...
                                                    $link_add_action = site_url($action->link_url . $row->primary_key_value);
                                                    if ($action->type == 'emodal_type') {
                                                        ?>
                                            <li><a href="javascript:void(0)" onclick="emodal('<?php echo $link_add_action ?>','<?php echo $action->label ?>')"><i class="fa <?php echo $action->css_class; ?>"></i> <?php echo $action->label ?></a></li>
                                        <?php
                                                            } else if ($action->type == 'confirm_type') {
                                                                ?>
                                            <li><a href="javascript:void(0)" onclick="confirm_multi_button('<?php echo $link_add_action ?>','<?php echo $action->label ?>','<?php echo $action->message ?>')"><i class="fa <?php echo $action->css_class; ?>"></i> <?php echo $action->label ?></a></li>
                                        <?php
                                                            } else if ($action->type == 'ajax_type') {
                                                                ?>
                                            <li><a href="javascript:void(0)" onclick="ajaxModal('<?php echo $link_add_action ?>','<?php echo $action->label ?>','<?php echo $action->message ?>')"><i class="fa <?php echo $action->css_class; ?>"></i> <?php echo $action->label ?></a></li>
                                        <?php
                                                            } else {
                                                                ?>
                                            <li>
                                                <a href="<?php echo $link_add_action; ?>">
                                ...
                                <?php if (!$unset_read) { ?>
                                    <li>
                                        <a href="<?php echo $row->read_url ?>"><i class="fa fa-eye"></i>
                                            <?php echo $this->l('list_view') ?></a>
                                    </li>
                                <?php } ?>
                                <?php if (!$unset_delete) { ?>
                                    <li>
        ...
        <?php foreach ($columns as $column) { 
            $isUpload=(@$types{$column->field_name}->crud_type=='upload_file')?"style='text-align:center'":"";
            ?>
            <td <?php echo @$isUpload; ?> >
